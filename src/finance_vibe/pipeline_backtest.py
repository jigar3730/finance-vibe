...
# =====================================================================
# SCALED-OUT SIMULATOR (gap/slippage aware, 50% at 1R, BE runner to 2R)
# =====================================================================
...
def _stop_exit_price(is_long: bool, stop: float, open_px: Optional[float], slippage_pct: float) -> float:
...
def simulate_scaled_trade(
    ...
    *,
    ...
    """Gap/slippage-aware scale-out simulation.

    Model: fill on a pullback limit; take ``partial_fraction`` off at 1R
    (``target1``) and move the remaining stop to breakeven; run the remainder
    to 2R (``target2``). Blended R combines both legs. Same-bar stop/target
    ambiguity is resolved pessimistically (stop assumed first).
    """
    risk = abs(entry - stop)
    result = {
        "outcome": "no_fill", "fill_index": None, "exit_index": None,
        "fill_date": None, "fill_price": None, "gap_entry": False,
        "stop_moved_be": False,
        "partial_date": None, "partial_price": None, "partial_r": None,
        "runner_date": None, "runner_price": None, "runner_r": None,
        "blended_r": None, "bars_held": 0, "mae_r": None, "mfe_r": None,
        "risk": risk,
    }
    if risk <= 0:
        return result
    ...
    for k in range(fill_idx, end):
        ...
        if not partialed:
            ...
            if stop_hit:  # pessimistic: full stop before any partial
                px = _stop_exit_price(is_long, stop, o, slippage_pct)
                full_r = r_of(px)
            ...
            if t1_hit:
                ...
                partial_date = date
                partialed = True
                be_stop = fill_price  # remaining risk moved to breakeven
                # same-bar resolution for the runner
                be_hit = lo <= be_stop if is_long else hi >= be_stop
                t2_hit = hi >= target2 if is_long else lo <= target2
                if be_hit:  # pessimistic: breakeven before 2R
                    px = _stop_exit_price(is_long, be_stop, o, slippage_pct)
    ...
    if outcome is None:  # max hold reached
        ...
        exit_index = last_idx
        if partialed:
            runner_price, runner_r, runner_date = last_close, r_of(last_close), last_date
...
def backtest_ticker(
    ...
    Enforces one position at a time (no overlap), long-only when the profile
    requires it, a directional macro gate, and cooldown measured from the last
    *exit* (unfilled signals never consume cooldown).
    """
    ...
    if partial_fraction is None:
        partial_fraction = float(sp.get("partial_fraction") or config.BACKTEST_PARTIAL_FRACTION)
    ...
    for i in range(warmup, len(df) - 1):
        if i <= position_exit_idx:
            continue  # a position is still open; no overlapping entries
        ...
        res = simulate_scaled_trade(
            df, i + 1, is_long, entry, stop, t1, t2, entry_valid, max_hold,
            slippage_pct=slippage_pct, partial_fraction=partial_fraction,
        )

        if res["outcome"] == "no_fill":
            counts["no_fill"] += 1
            continue  # never filled -> no position and no cooldown consumed
        ...
        trades.append({
            ...
            "Fill Price": round(res["fill_price"], 2) if res["fill_price"] is not None else None,
            "Gap Entry": res["gap_entry"],
            "Stop Moved BE": res["stop_moved_be"],
            "Partial Exit Date": _fmt_date(res["partial_date"]),
            "Partial Price": round(res["partial_price"], 2) if res["partial_price"] is not None else None,
            "Partial R": round(res["partial_r"], 2) if res["partial_r"] is not None else None,
            "Runner Exit Date": _fmt_date(res["runner_date"]),
            "Runner Price": round(res["runner_price"], 2) if res["runner_price"] is not None else None,
            "Runner R": round(res["runner_r"], 2) if res["runner_r"] is not None else None,
            "Outcome": res["outcome"],
            "Blended R Multiple": round(blended, 2) if blended is not None else None,
...
def print_summary(all_counts: dict, trades_df: pd.DataFrame) -> None:
    """Print risk-based backtest statistics (centered on filled, blended-R trades)."""
    print("\n" + "=" * 60)
    ...
    print(f"  Expired (no partial): {all_counts.get('expired_no_partial', 0)}")
    ...
    def _print_block(label: str, frame: pd.DataFrame) -> None:
        ...
        print(f"\n{label}:")
        print(f"  Trades:        {s['n']}")
        print(f"  Win rate:      {s['win_rate']:.1f}%  (95% CI {s['win_lo']:.0f}-{s['win_hi']:.0f}%)")
        print(f"  Expectancy:    {s['expectancy']:+.2f}R  (total {s['total_r']:+.2f}R)")
        print(f"  Avg winner:    {s['avg_winner']:+.2f}R   Avg loser: {s['avg_loser']:+.2f}R")
        print(f"  Profit factor: {pf}")
        if "MAE R" in frame.columns and frame["MAE R"].notna().any():
            print(f"  Avg MAE/MFE:   {frame['MAE R'].mean():+.2f}R / {frame['MFE R'].mean():+.2f}R")
...
def run_backtest(
    ...
    ``mode`` may be a data timeframe (``weekly``/``daily``) or ``high_beta``
    (daily OHLCV + high-beta swing profile).
    """
    ...
    logs_dir = config.get_log_dir(mode)  # isolated per profile (high_beta silo)
    ticker_filter = load_ticker_filter(mode_cfg, tickers)

    # Mode-aware defaults from quality-swing profile (CLI overrides still win)
    ...
    # Load benchmark once for regime / relative-strength gating (no per-bar reload).
    benchmark_df = None
    if swing.get("benchmark"):
        benchmark_df = load_benchmark_frame(swing["benchmark"], data_mode)
        if benchmark_df is None:
            print(f"WARNING: benchmark {swing['benchmark']} unavailable; regime/RS gates will reject all.")

    all_trades: list[dict] = []
    all_counts: dict = {}

    target_desc = (
        f"T1/T2={swing['t1_r']}/{swing['t2_r']}R" if swing.get("use_r_targets")
        else f"T1/T2={swing['t1_atr']}/{swing['t2_atr']}ATR"
    )
    ...
    print(
        f"Profile: {target_desc} long_only={swing.get('long_only')} "
    ...
    print(f"Execution: slippage={config.BACKTEST_SLIPPAGE_PCT*100:.2f}% "
          f"partial={swing.get('partial_fraction')} (50% at 1R, BE runner to 2R)")
    print(f"Raw dir: {raw_dir}")
    print(f"Tickers: {len(paths)}\n")

    for path in paths:
        trades, counts = backtest_ticker(
            path, swing_profile, long_min, short_max, warmup, entry_valid, max_hold,
            cooldown_bars=cooldown_bars, benchmark_df=benchmark_df,
        )
        ...
        if trades:
            symbol = ticker_from_filename(path)
            print(f"{symbol}: {counts.get('signals', 0)} signal(s), {len(trades)} filled")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--long-min-score", type=int, default=None,
    ...
    if args.long_min_score is None:
        # Soft vibe_min lives in detect_setup_at_bar for daily/high_beta.
